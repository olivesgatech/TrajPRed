...
class TrajPRed_re_NP(BaseModel):
    def __init__(self, cfg):
        ...
        if not cfg.point_rel_enable:
            if cfg.region_rel_backbone  ==  'GRU':
                self.temp_rel_enc = nn.GRU(input_size=self.temp_enc.hidden_size+2, hidden_size=self.temp_enc.hidden_size, batch_first=True)
            else:
    ...
    def forward(self, input_x, 
        ...
        if self.cfg.tempenc == 'rnn':
            ...
            for row in range(H):
                for col in range(W):
                    output, _ = self.temp_enc(hist_cae_z[:, :, :, row, col])
