""" 
generating density maps for Standford Drone Dataset trajectories
"""
import os
import glob
import argparse
...
class DensityMaps:
    ...
    def generate_density_maps(self, annot_path, out_dist_path, min_sigma = 2, method = 1, map_out_folder = 'maps_adaptive_kernel/'):
        ...
        frame_col = annot.query('lost!=1')[['frame',]].values
        pos_x = (annot.query('lost!=1')[['xmin', 'xmax']].values).mean(1)
        pos_y = (annot.query('lost!=1')[['ymin', 'ymax']].values).mean(1)
        data = np.hstack((frame_col, pos_x.reshape(-1,1), pos_y.reshape(-1,1)))

        # data = self.read_txt_file(self.args.data_pixel_path)  # all trajectory data
        frames = np.unique(data[:, 0]).tolist()
        curr_map_out_folder = '/'.join(annot_path.replace('annotations', map_out_folder.replace('/','')).split('/')[:-1])
        ...
        for frame in tqdm(frames):
            ...
            img_path = annot_path.replace('annotations.txt', 'reference.jpg')
            full_img_path = out_dist_path.replace('distances_dict.pkl', f'{int(frame)}')

            # load img and map
            img = plt.imread(img_path)
            height, width  = img.shape[0],  img.shape[1]

            gt_points = self.get_gt_dots(data[frame == data[:, 0], 1:], *img.shape[0:2])
            ...
            # save 
            gt_out_path = os.path.join(curr_map_out_folder , f'{int(frame)}.ftr')
            pf.write_feather(pd.DataFrame(list(zip([density_map.tolist(),])), columns=['density_map']), gt_out_path)
    ...
    def compute_distances(self, annot_path, out_dist_path='distances_dict.pkl', img_path='./', n_neighbors = 4, leafsize=1024):
        distances_dict = dict()
        annot = pd.read_csv(annot_path, sep=' ', names=['TrackID', 'xmin', 'ymin', 'xmax', 'ymax', 'frame', 'lost', 'occluded', 'generated', 'label'])
        # _coords_curr = annot.query('lost!=1')[['frame', 'xmin','ymin','xmax','ymax']].values
        frame_col = annot.query('lost!=1')[['frame',]].values
        pos_x = (annot.query('lost!=1')[['xmin', 'xmax']].values).mean(1)
        pos_y = (annot.query('lost!=1')[['ymin', 'ymax']].values).mean(1)
        data = np.hstack((frame_col, pos_x.reshape(-1,1), pos_y.reshape(-1,1)))
        ...
        for frame in tqdm(frames):
            # mat_path = full_img_path.replace('.jpg','.mat').replace('images','ground_truth').replace('IMG_','GT_IMG_')
            full_img_path = out_dist_path.replace('distances_dict.pkl', f'{int(frame)}')
            # full_img_path = os.path.join(self.args.img_path,f'{int(frame)}')
            img = plt.imread(img_path)

            non_zero_points = self.get_gt_dots(data[frame == data[:, 0], 1:], *img.shape[0:2])
        ...
        os.makedirs(out_dist_path.replace('distances_dict.pkl',''), exist_ok=True)
...
def parse_args():
    ...
    parser.add_argument('--datalist_root', default='', type=str)
...
def worker(vid, dm, args):
    
    print(f'reading {os.path.splitext(vid)[0]}')
    annot_path = os.path.join(args.annot_root, os.path.splitext(vid)[0].replace('_','/'), 'annotations.txt')
    img_path = annot_path.replace('annotations.txt', 'reference.jpg')
    # annot_path = '/'.join(['./sdd']+annot_path.split('/')[-3:])
    # different dictionaries to store different scenes
    precomputed_distances_path = ('/'.join(['./sdd']+annot_path.split('/')[-3:])).replace('annotations.txt', 'distances_dict.pkl')

    # uncomment to generate and save dict with distances 
    dm.compute_distances(annot_path = annot_path, out_dist_path=precomputed_distances_path, img_path=img_path)

    dm.load_distances_dict(precomputed_distances_path)

    dm.generate_density_maps(annot_path = annot_path, out_dist_path=precomputed_distances_path, method = 3, map_out_folder = 'maps_fixed_kernel/')
...
if __name__ == '__main__':
    ...
    precomputed_kernels_path = './sdd/gaussian_kernels.pkl'
    ...
    for subset in ['train','val','test']:
        datalist_path = os.path.join(args.datalist_root, f'{subset}.lst')
        vid_list = np.hstack(pd.read_csv(datalist_path, header=None).values).tolist()
        ...
        with mp.Pool(mp.cpu_count()) as pool:
            with tqdm(total=len(work)) as pbar:
                for _ in pool.imap_unordered(partial(worker, dm=dm, args=args), work):
                    pbar.update()