...
class ETHUCYDataset(data.Dataset):
    def __init__(self, cfg, split, transforms=None):
        ...
        conf_json = open(cfg.dataset.ethucy_config, 'r')
...
#                 all_obs_distance.mean(), all_obs_distance.std(), all_obs_vel.mean(), all_obs_vel.std(),
    ...
    def _getnode_(self, node_timestep_data):
        ...
        # print("original frame dim: ", height, width) 
        ...
        if self.cfg.dataset.perturb_input and self.cfg.test.inference:
            x_uv_t = self.to_image_frame(np.linalg.inv(homog), x_t[:,:2]+0.1*np.random.randn(x_t[:,:2].shape[0], x_t[:,:2].shape[1]))
        else:
            x_uv_t = self.to_image_frame(np.linalg.inv(homog), x_t[:,:2])
        y_uv_t = self.to_image_frame(np.linalg.inv(homog), y_t[:,:2])
        if self.cfg.model.point_rel_enable: ## transform neighboring nodes
            for i, neighbor in enumerate(neighbors_data[self.dataset.edge_types[0]]): # list of neighbors...# x_uv_t = self.to_image_frame(np.linalg.inv(homog), x_t[:,:2])
                neighbor_uv = self.to_image_frame(np.linalg.inv(homog), neighbor[:,:2].numpy())
                if self.cfg.dataset.normalize == 'zero-one':  #Normalize agent states
                    ...
                    neighbor_uv[:, 0], neighbor_uv[:, 1] = neighbor_uv[:, 0]*self.cfg.model.convae.image_size/width, neighbor_uv[:, 1]*self.cfg.model.convae.image_size/height
                # then derivative
        ...
        if self.cfg.dataset.normalize == 'zero-one':  #Normalize agent states
            ...
            # normalize to same dim as density maps
            x_uv_t[:, 0], x_uv_t[:, 1] = x_uv_t[:, 0]*self.cfg.model.convae.image_size/width, x_uv_t[:, 1]*self.cfg.model.convae.image_size/height
            y_uv_t[:, 0], y_uv_t[:, 1] = y_uv_t[:, 0]*self.cfg.model.convae.image_size/width, y_uv_t[:, 1]*self.cfg.model.convae.image_size/height
        ...
        if self.cfg.dataset.normalize == 'zero-one':
            ret['input_x_frame'] = torch.from_numpy(x_uv_t*self.cfg.model.convae.image_size)
        else:
            ret['input_x_frame'] = torch.from_numpy(x_uv_t)
        # for post-processing
        ...
        if self.cfg.dataset.perturb_input and self.cfg.test.inference:
            ...
            ret['input_imgs'] = self.create_input_maps(traj_data,timestep,timestep_min,height,width, homog=homog, perturb_input=self.cfg.dataset.perturb_input)
    ...
    def load_density_maps(self,path,timestep,timestep_min,fhi):
        ...
        # load input density
        input_density_maps = []
        for i, frame in enumerate(range((timestep_min+timestep-self.cfg.model.input_len+1)*10, (timestep_min+timestep+1)*10, 10)):
            # print(i, frame)
        ...
        if self.cfg.dataset.excl_unseen_node:
            ...
            target_density_maps = []
            for i, frame in enumerate(range((timestep_min+timestep+1)*10, (timestep_min+timestep+self.cfg.model.pred_len+1)*10, 10)):
                # print(i, frame)
    ...
    def _load_transformed_dms(self, dm_root, frame, in_channel=1):
        ...
        cache_path = os.path.join(cache_root,f'{int(frame):05d}.pkl')
        # print(cache_path)
        if not os.path.exists(cache_path): 
            os.makedirs(cache_root, exist_ok=True)
            # load raw density maps
            with h5py.File(os.path.join(dm_root,f'{int(frame):05d}.h5'), 'r') as hf:
    ...
    def create_target_maps(self,traj_data,timestep,timestep_min,height,width,in_channel=1):
        target_maps = []
        input_frames = list(range((timestep_min+timestep-self.cfg.model.input_len+1)*10, (timestep_min+timestep+1)*10, 10))
...
#             if len(_traj_target_df)==0: raise ValueError('A very specific bad thing happened.')
    ...
    def create_input_maps(self,traj_data,timestep,timestep_min,height,width,homog,perturb_input,in_channel=1):
        target_maps = []
        input_frames = list(range((timestep_min+timestep-self.cfg.model.input_len+1)*10, (timestep_min+timestep+1)*10, 10))
        # input_nodeIDs = np.unique(traj_data.query('frameID in @input_frames').nodeID)
        for i, frame in enumerate(input_frames):
            ...
            if perturb_input:
                x_uv_t = self.to_image_frame(np.linalg.inv(homog), _traj_target_df[['u','v']].values + 0.1*np.random.randn(_traj_target_df[['u','v']].values.shape[0], _traj_target_df[['u','v']].values.shape[1]))

            else:
                x_uv_t = self.to_image_frame(np.linalg.inv(homog), _traj_target_df[['u','v']].values)
...
if __name__=='__main__':
    dataset = ETHUCYDataset(cfg, split)
